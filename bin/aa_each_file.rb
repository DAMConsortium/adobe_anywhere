...
op.on('--production-name NAME', 'A production name to add the assets to.') { options[:production_name] = v }
...
def aa_execute(*args)
  ...
  method_arguments_json = %({ "production_name": "#{production_name}", "media_paths": "eamedia://#{_file_path}", "create_asset_if_not_exists":true })
